...
export default {
  ...
  methods: {
    ...
    async getPresenceData(whereStartTime, whereEndTime) {
      ...
      const query = `${process.env.INFLUX_HOST}/query?pretty=true&db=tcc_data&q=SELECT COUNT(DISTINCT("srcMac")) FROM "probe_data" WHERE time >= '${moment(whereStartTime).toISOString()}' AND time <= '${moment(whereEndTime).toISOString()}' GROUP BY time(${selectMeanInterval}m), sensor`;
